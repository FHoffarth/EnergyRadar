...
import { RawSettings, LocationCandidateData, ProviderType, DemoPresetId, ThemeMode } from '../types';
import {
...
export function SettingsView() {
  ...
  const {
    ...
    selectDemoPreset, toggleDemoJitter,
  ...
  const availablePresets = getAvailableDemoPresets();
  const [activeDemoPreset, setActiveDemoPreset] = useState<DemoPresetId>('sunny_midday');
  const [jitterEnabled, setJitterEnabled] = useState(false);

  useEffect(() => {
    if (raw && !isDirty) {
      setDraft(raw);
      if (raw.location_query) setLocationInput(raw.location_query);
    }
  }, [raw, isDirty]);
  ...
  const handleSearchLocations = (e: React.FormEvent) => {
    e.preventDefault();
    if (locationInput.trim()) searchWeatherLocations(locationInput);
  };
  ...
  const handlePresetChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const id = e.target.value as DemoPresetId;
    setActiveDemoPreset(id);
    selectDemoPreset(id);
  };
  ...
  return (
    <div className="flex flex-col h-full pt-12 pb-12 overflow-y-auto px-12">
      <header className="pb-8 flex items-start justify-between">
        <div>
          <h1 className="text-4xl font-semibold text-[#1C1C1E] dark:text-white leading-tight">Einstellungen</h1>
          <p className="text-[#6E6E6E] dark:text-slate-400 mt-2 text-lg">Verwalte Darstellung, Standort, Wetter und Systemoptionen.</p>
        </div>
      ...
      <div className="space-y-8 max-w-4xl">
        {/* Provider Selection */}
        <section className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-[#E5E5E3] dark:border-slate-800 shadow-sm space-y-5">
          <div className="flex items-center gap-3 pb-4 border-b border-[#E5E5E3] dark:border-slate-800">
            <Database className="w-6 h-6 text-indigo-500" />
            <h2 className="text-xl font-bold text-[#1C1C1E] dark:text-white">Datenanbieter</h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            <label className={`p-3 rounded-xl border cursor-pointer transition-all ${
              providerType === 'demo'
                ? 'bg-sky-50 text-sky-900 border-sky-300 dark:bg-sky-950/80 dark:text-sky-200 dark:border-sky-500/50 shadow-xs'
                : 'bg-slate-50 border-slate-200 text-slate-700 dark:bg-slate-800/30 dark:border-slate-700/50 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'
            }`}>
              <input type="radio" name="providerType" checked={providerType === 'demo'}
                onChange={() => handleProviderChange('demo')} className="hidden" />
              <span className="font-semibold block">Demo-Modus</span>
              <span className="text-[11px] opacity-80 block mt-0.5">Simulierte Szenarien für Vorführung und Test</span>
            </label>
            <label className={`p-3 rounded-xl border cursor-pointer transition-all ${
              providerType === 'bridge'
                ? 'bg-emerald-50 text-emerald-900 border-emerald-300 dark:bg-emerald-950/80 dark:text-emerald-200 dark:border-emerald-500/50 shadow-xs'
                : 'bg-slate-50 border-slate-200 text-slate-700 dark:bg-slate-800/30 dark:border-slate-700/50 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'
            }`}>
              <input type="radio" name="providerType" checked={providerType === 'bridge'}
                onChange={() => handleProviderChange('bridge')} className="hidden" />
              <span className="font-semibold block">Desktop-Bridge</span>
              <span className="text-[11px] opacity-80 block mt-0.5">Echte Live-Daten über die Desktop-Anbindung</span>
            </label>
          </div>
            ...
            <div className="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-800/60">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-800 dark:text-slate-200">Demo-Szenario</label>
                  <select value={activeDemoPreset}
                    onChange={handlePresetChange}
                    className="w-full px-4 py-2.5 rounded-xl border border-[#E5E5E3] dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500 cursor-pointer">
                    {availablePresets.map(p => (
                      <option key={p.id} value={p.id}>{p.name}</option>
                    ))}
                  </select>
                </div>
                <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/30 border border-[#E5E5E3] dark:border-slate-700/50">
                  <div>
                    <span className="font-semibold text-slate-900 dark:text-slate-100 block text-sm">Telemetrie-Jitter</span>
                    <span className="text-[11px] text-slate-500 dark:text-slate-400 block mt-0.5">Leichte Schwankungen simulieren</span>
                  </div>
                  <button onClick={handleJitterToggle}
                    className="text-sky-600 dark:text-sky-400 hover:text-sky-700">
                    {jitterEnabled ? <ToggleRight className="w-7 h-7" /> : <ToggleLeft className="w-7 h-7 text-slate-400" />}
                  </button>
                </div>
              </div>
            </div>
          ...
          {providerType === 'bridge' && (
            <div className="p-4 rounded-2xl bg-sky-50 dark:bg-sky-950/30 border border-sky-200 dark:border-sky-800/50 text-xs text-sky-800 dark:text-sky-300 leading-relaxed">
              <strong>Hinweis:</strong> Die Desktop-Bridge verbindet sich mit der EnergyRadar Desktop-Anwendung über Qt WebChannel.
              Der Bridge-Modus zeigt ausschließlich Daten, die von der Desktop-Anwendung bereitgestellt werden.
            </div>
          )}
        </section>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-[#E5E5E3] dark:border-slate-800 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6 pt-2 border-t border-slate-100 dark:border-slate-800/60">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-3">
                {[{ id: 'de-DE', label: '1.234,56 (Deutsch)' }, { id: 'en-US', label: '1,234.56 (International)' }].map(n => (
        ...
        <section className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-[#E5E5E3] dark:border-slate-800 shadow-sm space-y-6">
          <div className="flex items-center gap-3 pb-4 border-b border-[#E5E5E3] dark:border-slate-800">
            ...
            <h2 className="text-xl font-bold text-[#1C1C1E] dark:text-white">Standort & Wetter</h2>
          </div>
          ...
          <form onSubmit={handleSearchLocations} className="space-y-3">
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={isSearchingCandidates || !locationInput.trim()}
                ...
                {isSearchingCandidates ? <><Loader2 className="w-4 h-4 animate-spin" /><span>Suchen...</span></> : <span>Suchen</span>}
              </button>
            </div>
          </form>
              ...
              <div className="space-y-2">
                {weatherCandidates.map((cand, idx) => (
                  <div key={cand.provider_id || idx}
                    ...
                    <button onClick={() => confirmWeatherLocation(cand)}
            ...
            <div className="p-5 rounded-2xl border border-emerald-200 dark:border-emerald-800/80 bg-emerald-50/40 dark:bg-emerald-950/20 flex items-start justify-between">
              <div className="space-y-1">
                ...
                <p className="text-xs font-mono text-slate-600 dark:text-slate-400">
                  Koordinaten: {resLoc.latitude.toFixed(4)}, {resLoc.longitude.toFixed(4)} · Zeitzone: {resLoc.timezone || 'Europe/Berlin'}
                </p>
              </div>
              <button onClick={removeResolvedLocation}
                ...
                <Trash2 className="w-3.5 h-3.5" /> Standort entfernen
              </button>
            </div>
          ) : (
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/60">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Ruft automatische Wetter- und Sonnendaten ab.</p>
            </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-slate-500 font-medium">kWp Peak-Leistung</span>
            </div>
          </div>

          <div className="pt-2 border-t border-slate-100 dark:border-slate-800/60 space-y-4">
            <div className="flex items-center justify-between">
              <button onClick={testWeatherConnection} disabled={weatherTestState.testing || !resLoc}
                ...
                {weatherTestState.testing ? <><Loader2 className="w-4 h-4 animate-spin" /><span>Führe Live-Verbindungstest aus...</span></>
                  : <><CloudRain className="w-4 h-4" /><span>Wetterverbindung testen</span></>}
              </button>
              <span className="text-xs text-slate-400 flex items-center gap-1">
                <Globe className="w-3.5 h-3.5" /> Data by Open-Meteo (CC BY 4.0)
              </span>
            </div>
            ...
            {weatherTestState.result && (
              <div className={`p-5 rounded-2xl border space-y-3 ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-mono text-slate-500">Latenz: {weatherTestState.result.latency_ms} ms</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800/60">
            <div className="flex items-center gap-3">
              <button onClick={openDiagnosticLog}
                ...
                <FileText className="w-4 h-4 text-emerald-500" /> Diagnoseprotokoll öffnen
              </button>
              <button onClick={openLogDirectory}
                className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-xl font-medium text-sm flex items-center gap-2 transition-colors">
                <Folder className="w-4 h-4 text-slate-500" /> Log-Ordner öffnen
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input type="text" placeholder="z. B. wechselrichter.local"
                  value={getEff('fronius_address') ?? ''}
                  onChange={(e) => updateDraft('fronius_address', e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Enter') { const addr = getEff('fronius_address'); if (addr) saveFroniusAddress(addr); testConnection('fronius_primary'); } }}
                ...
                <button type="button"
                  onClick={() => { const addr = getEff('fronius_address'); if (addr) saveFroniusAddress(addr); testConnection('fronius_primary'); }}
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input type="text" placeholder="z. B. zaehler.local"
                  value={getEff('mt175_address') ?? ''}
                  onChange={(e) => updateDraft('mt175_address', e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Enter') { const addr = getEff('mt175_address'); updateSettings({ mt175_address: addr }); testConnection('mt175_primary'); } }}
                  className="flex-1 px-4 py-2.5 rounded-xl border border-[#E5E5E3] dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" />
                <button type="button"
                  onClick={() => { const addr = getEff('mt175_address'); updateSettings({ mt175_address: addr }); testConnection('mt175_primary'); }}
