...
export function EnergyFlow({ snapshot }: EnergyFlowProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/70 p-4 sm:p-5 shadow-sm font-sans">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {solar.origin === 'observed' && homeLoad.origin === 'unavailable' && grid.origin === 'unavailable'
              ? 'PV-Daten live, Verbrauch und Netz nicht verfügbar'
             : solar.origin === 'observed'
              ? 'Verifizierte Live-Telemetriewerte'
             : solar.origin === 'simulated'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        <div className={`p-3 rounded-xl border transition-colors ${
          solar.valueKw !== null && solar.valueKw > 0
            ...
            : 'bg-slate-50/50 border-slate-200/50 dark:bg-slate-800/20 dark:border-slate-700/30'
        }`}>
        ...
        <div className={`p-3 rounded-xl border transition-colors ${
          ...
          <div className="flex items-center justify-between mb-1.5">
            <div className={`p-1.5 rounded-lg ${isCharging ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-sky-500/10 text-sky-600 dark:text-sky-400'}`}>
              <Battery className="w-4 h-4" />
            </div>
            {battery ? renderOriginBadge(battery.origin) : <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100/50 text-slate-500 border border-slate-200/50 dark:bg-slate-800/30 dark:text-slate-400 dark:border-slate-700/30 font-medium">Nicht verfügbar</span>}
          </div>
          <div className="text-xl font-bold tabular-nums tracking-tight text-slate-900 dark:text-slate-100">
        ...
        <div className="p-3 rounded-xl bg-indigo-50/80 border border-indigo-200/80 dark:bg-indigo-950/10 dark:border-indigo-500/20">
          <div className="flex items-center justify-between mb-1.5">
        ...
        <div className={`p-3 rounded-xl border transition-colors ${
          ...
          <div className="flex items-center justify-between mb-1.5">
            <div className={`p-1.5 rounded-lg ${isImporting ? 'bg-rose-500/10 text-rose-600 dark:text-rose-400' : 'bg-sky-500/10 text-sky-600 dark:text-sky-400'}`}>
